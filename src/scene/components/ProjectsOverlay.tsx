...
function solveHomography(source:readonly {x:number;y:number}[],destination:readonly {x:number;y:number}[]){
  ...
  for(let pivot=0;pivot<8;pivot++){
    let row=pivot;
    for(let candidate=pivot+1;candidate<8;candidate++)if(Math.abs(matrix[candidate][pivot])>Math.abs(matrix[row][pivot]))row=candidate;
    [matrix[pivot],matrix[row]]=[matrix[row],matrix[pivot]];[values[pivot],values[row]]=[values[row],values[pivot]];
    ...
    for(let candidate=0;candidate<8;candidate++){
      ...
      for(let column=pivot;column<8;column++)matrix[candidate][column]-=factor*matrix[pivot][column];
...
const EXPERIENCE: readonly ExperienceEntry[] = [
  { role: "Product Manager for Brain Studio & Connect", company: "Jelou", dates: "December 2025 – Present", location: "Guayaquil, Ecuador" },
  {
  ...
  { role: "Software Engineer", company: "Pacificsoft S.A.", dates: "May 2017 – June 2017", location: "Guayaquil", bullets: ["Used Angular and .NET to develop a module for an airline reservation system."] },
  {
  ...
  { role: "Academic Assistant – HCI Course", company: "Escuela Superior Politécnica del Litoral", dates: "November 2014 – March 2015", location: "Guayaquil", bullets: ["Assisted in Android development labs covering UI layouts, SQLite, Google Maps API v2, and activity transitions."] },
  { role: "Technical Support Assistant", company: "Centro de Emprendedores, Escuela Superior Politécnica del Litoral", dates: "June 2014 – March 2015", location: "Guayaquil", bullets: ["Implemented multi-site WordPress platform for Student Clubs and Professional Associations."] },
  { role: "Intern", company: "Blindside Networks", dates: "March 2014 – May 2014", location: "Guayaquil", bullets: ["Built prototypes for real-time chat and video-streaming tests using Node.js, Socket.io, and Kurento Media Framework."] },
  { role: "Intern", company: "Blindside Networks", dates: "March 2013 – May 2013", location: "Guayaquil", bullets: ["Enhanced platform usability by updating user interface components and refining front-end interactions based on user feedback and platform standards."] },
];
...
export function ProjectsOverlay({ visible,projectionRef }: { visible: boolean; projectionRef:ScreenProjectionRef }) {
  ...
  useEffect(()=>{
    if(!visible)return;
    const source=[{x:0,y:0},{x:SCREEN_LOGICAL_WIDTH,y:0},{x:SCREEN_LOGICAL_WIDTH,y:SCREEN_LOGICAL_HEIGHT},{x:0,y:SCREEN_LOGICAL_HEIGHT}];
    let frame=0;
    const update=()=>{
      const shell=shellRef.current,projection=projectionRef.current;
      if(shell&&projection){
        const transform=solveHomography(source,projection.points);
        if(transform){shell.style.transform=transform;shell.style.visibility="visible";}
      }
  ...
  return <section className={`projects-overlay${visible?"":" is-exiting"}`} role="dialog" aria-modal="true" aria-label="Experience and projects">
    <div ref={shellRef} className="projects-overlay-shell" style={{width:SCREEN_LOGICAL_WIDTH,height:SCREEN_LOGICAL_HEIGHT,maxHeight:"none",visibility:"hidden"}}>
      ...
      <div className="projects-overlay-columns">
        ...
        <article className="projects-overlay-column projects-overlay-projects">
          ...
          <div className="projects-overlay-project-list">{PROJECTS.map((project)=><ProjectLink key={project.title} project={project}/>)}</div>
        </article>
      </div>
    </div>
  </section>;
