...
export interface SceneRouteState extends NavigationLocation {
  path: string;
  shot: ShotId;
  target: ShotId;
  section?: string;
...
export function resolveNavigationPath(pathname: string): NavigationLocation {
  ...
  if (parts[0] === "projects")
    return parts[1]
      ? (locationForFocus("projects", parts[1]) ?? locationForScene("projects"))
      : locationForScene("projects");
  if (parts[0] === "certificates")
    return parts[1]
      ? (locationForFocus("certificates", parts[1]) ?? {
          sceneId: "certificates",
          focusCollectionId: "certificates",
          focusItemId: parts[1],
          cameraTarget: SCENE_REGISTRY.certificates.cameraTarget,
        })
      : locationForScene("certificates");
  if (parts[0] === "poems")
    return parts[1]
      ? (locationForFocus("poems", parts[1]) ?? {
          sceneId: "poems",
          focusCollectionId: "poems",
          focusItemId: parts[1],
  ...
  if (parts[0] === "wall")
    return parts[1]
      ? (locationForFocus("wall", parts[1]) ?? locationForScene("wall"))
      : locationForScene("wall");
  return locationForScene("opening", "workspace");
}

export function shotForRoute(pathname: string): ShotId {
  return resolveNavigationPath(pathname).cameraTarget;
}
...
export function parseScenePath(pathname: string): SceneRouteState {
  ...
  return {
    ...location,
    path: pathname,
    shot: location.cameraTarget,
    target: location.cameraTarget,
    section: parts[0],
    slug: parts[1],
    directEntry: pathname !== "/",
  };
}

export const SCENE_ROUTES = Object.fromEntries(
  Object.values(SCENE_REGISTRY)
    .filter((scene) => scene.route !== null)
    .map((scene) => [
      scene.subject,
      {
        path: scene.route!,
        target: scene.cameraTarget,
        shot: scene.cameraTarget,
        sceneId: scene.id,
      },
    ]),
);
export const STATIC_FOCUS_ROUTES = Object.values(FOCUS_COLLECTIONS).flatMap((collection) =>
  Object.values(collection.items).map((item) => item.route),
);
