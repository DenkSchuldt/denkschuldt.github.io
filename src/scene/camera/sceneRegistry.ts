...
import { POEMS_FOLDER_LAYOUT, poemsAlignedCameraPosition } from "../sceneLayout.ts";
...
export const SCENE_REGISTRY:Record<SceneId,SceneDefinition>={
  ...
  about:{id:"about",label:"About me",route:"/about",subject:"paper",cameraTarget:"about",framing:framing([-1.8,3,-.772],[-2,1.25,-1.022],31,{roll:-25,waypoint:[-1.6,2.85,.16],composition:"readable manuscript"}),cameraFocus:{enabled:true,focusDistance:.013,depthOfFieldStrength:0,focusTarget:"paper"},transition:{duration:4.3},responsive:{mobile:{position:[-1.897,3.16,-.578],fov:46,roll:0}}},
  ...
  wall:{id:"wall",label:"Wall",route:"/wall",subject:"wall",cameraTarget:"wall",framing:framing([4.65,3.05,2.45],[2.6,2.85,-3.75],40,{waypoint:[3.8,3.1,3.4]}),cameraFocus:{enabled:true,focusDistance:.028},transition:{duration:5.2,breathing:quietBreathing},responsive:{mobile:{position:[4.1,3.15,3.3],fov:48}},focusCollection:"wall"},
  phone:{id:"phone",label:"Phone",route:"/phone",subject:"phone",cameraTarget:"phone",framing:framing([.25,2.78,.72],[-.25,1.26,-.73],33,{waypoint:[1.05,2.65,2.1]}),cameraFocus:{enabled:true,focusDistance:.011,focusTarget:"phone-screen"},transition:{duration:4.2,breathing:{...quietBreathing,positionAmplitude:.0008}},responsive:{mobile:{position:[.1,3.1,1.15],fov:41}},focusCollection:"phone"},
  poems:{id:"poems",label:"Poems",route:"/poems",subject:"folder",cameraTarget:"poems",framing:framing(poemsAlignedCameraPosition(3.38,.3),POEMS_FOLDER_LAYOUT.worldCenter,30,{waypoint:poemsAlignedCameraPosition(3.35,1.4),composition:"close stable overhead reading view aligned with writing portfolio"}),cameraFocus:{enabled:true,focusDistance:.012,depthOfFieldStrength:0,focusTarget:"folder-pages"},transition:{duration:4.8,breathing:{positionAmplitude:.00035,rotationAmplitude:.00008,speed:.18}},responsive:{mobile:{position:poemsAlignedCameraPosition(4.65,.24),lookAt:POEMS_FOLDER_LAYOUT.worldCenter,fov:50},tablet:{position:poemsAlignedCameraPosition(3.82,.25),lookAt:POEMS_FOLDER_LAYOUT.worldCenter,fov:38}},focusCollection:"poems"},
...
const dynamicCollection=(id:"projects"|"wall"|"poems",sceneId:SceneId,cameraTarget:ShotId,routePattern:string):FocusCollectionDefinition=>({id,sceneId,routePattern,cameraTarget,defaultFraming:SCENE_REGISTRY[sceneId].framing,cameraFocus:SCENE_REGISTRY[sceneId].cameraFocus,transition:SCENE_REGISTRY[sceneId].transition,items:{},orderedItemIds:[],allowDynamicItems:true});
...
export const FOCUS_COLLECTIONS:Record<string,FocusCollectionDefinition>={
  certificates:{id:"certificates",sceneId:"certificates",routePattern:"/certificates/:slug",cameraTarget:"certificate-detail",defaultFraming:certificateDetailFraming,cameraFocus:certificateDetailFocus,transition:certificateDetailTransition,items:certificateItems,orderedItemIds:CERTIFICATES.map(({slug})=>slug)},
  ...
  poems:dynamicCollection("poems","poems","poem-detail","/poems/:slug"),
  phone:{id:"phone",sceneId:"phone",routePattern:"/phone/:slug",cameraTarget:"phone-qr",defaultFraming:SCENE_REGISTRY.phone.framing,cameraFocus:SCENE_REGISTRY.phone.cameraFocus,transition:SCENE_REGISTRY.phone.transition,items:phoneItems,orderedItemIds:["qr","socials"]},
};

export const GUIDED_SCENE_IDS:SceneId[]=["opening","about","certificates","projects","wall","phone","poems","drawer"];

export function getAdjacentScene(sceneId:SceneId,direction:-1|1):SceneId|null {
  const index=GUIDED_SCENE_IDS.indexOf(sceneId);
  if(index<0)return null;
  if(direction>0&&sceneId==="drawer")return "opening";
  return GUIDED_SCENE_IDS[index+direction]??null;
}
...
export function locationForFocus(collectionId:string,itemId:string):NavigationLocation|null {
  const collection=FOCUS_COLLECTIONS[collectionId];
  const item=getFocusItem(collectionId,itemId);
  return collection&&item?{sceneId:collection.sceneId,focusCollectionId:collection.id,focusItemId:item.id,cameraTarget:item.cameraTarget}:null;
}
