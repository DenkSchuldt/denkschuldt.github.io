...
const framing = (
  position: ShotFraming["position"],
  lookAt: ShotFraming["lookAt"],
  fov: number,
  extra: Partial<ShotFraming> = {},
): ShotFraming => ({ position, lookAt, fov, safeMargins: margins, alignment: "center", ...extra });
...
export const SCENE_REGISTRY: Record<SceneId, SceneDefinition> = {
  ...
  about: {
    ...
    subject: "paper",
    cameraTarget: "about",
    framing: framing([-1.8, 3, -0.772], [-2, 1.25, -1.022], 31, {
      roll: -25,
      waypoint: [-1.6, 2.85, 0.16],
      composition: "readable manuscript",
    }),
    cameraFocus: {
      enabled: true,
      focusDistance: 0.013,
  ...
  certificates: {
    ...
    framing: framing([-3.75, 2.3, 1.25], [-3.8, 2, -3.58], 44, {
      waypoint: [-1.8, 3.15, 2.35],
      composition: "chronological certificate archive",
    }),
    cameraFocus: { enabled: true, focusDistance: 0.026, focusTarget: "certificate" },
    transition: { duration: 5, breathing: quietBreathing },
    responsive: { mobile: { position: [-3.72, 2.35, 2.1], lookAt: [-3.8, 2, -3.58], fov: 52 } },
    focusCollection: "certificates",
  },
  projects: {
    id: "projects",
    label: "Projects",
    ...
    framing: framing([-0.25, 1.82, -0.25], [-0.55, 1.78, -2.3], 31, {
      composition: "interactive MacBook display",
    }),
  ...
  wall: {
    ...
    subject: "wall",
    cameraTarget: "wall",
    framing: framing([4.65, 3.05, 2.45], [2.6, 2.85, -3.75], 40, { waypoint: [3.8, 3.1, 3.4] }),
    cameraFocus: { enabled: true, focusDistance: 0.028 },
    transition: { duration: 5.2, breathing: quietBreathing },
    responsive: { mobile: { position: [4.1, 3.15, 3.3], fov: 48 } },
    focusCollection: "wall",
    autoAdvance: { to: "phone", delay: 0.18 },
  },
  phone: {
    id: "phone",
    label: "Phone",
    ...
    subject: "phone",
    cameraTarget: "phone",
    framing: framing(PHONE_LAYOUT.cameraPosition, PHONE_LAYOUT.cameraTarget, 29, {
      waypoint: [0.42, 2.95, 1.2],
      composition: "near-orthogonal scannable iPhone screen",
    }),
    cameraFocus: {
      enabled: true,
      focusDistance: 0.011,
      depthOfFieldStrength: 0,
      focusTarget: "phone-screen",
    },
    transition: { duration: 4.2, breathing: { ...quietBreathing, positionAmplitude: 0.0008 } },
    responsive: {
      mobile: {
  ...
  poems: {
    ...
    transition: {
      duration: 4.8,
      breathing: { positionAmplitude: 0.00035, rotationAmplitude: 0.00008, speed: 0.18 },
    },
    responsive: {
      mobile: {
  ...
  drawer: {
    ...
    subject: "drawer",
    cameraTarget: "drawer",
    framing: framing([2.85, 1.58, 1.45], [1.55, 0.72, -1.52], 36, { waypoint: [3.15, 1.95, 2.4] }),
    cameraFocus: { enabled: true, focusDistance: 0.014 },
    transition: {
      duration: 4.5,
      breathing: { positionAmplitude: 0.0007, rotationAmplitude: 0.0001, speed: 0.2 },
    },
    responsive: { mobile: { position: [2.55, 1.75, 2.15], fov: 43 } },
  },
};

export const WORKSPACE_PRESENTATION: Shot = {
  id: "workspace",
  label: "Workspace",
  route: "/",
  subject: "desk",
  framing: framing([-2.35, 3.25, 4.55], [-0.15, 1.45, -1.35], 44),
  focus: { enabled: true, focusDistance: 0.02 },
  transition: { duration: 4.8, breathing: quietBreathing },
  responsive: { mobile: { position: [-2.05, 3.25, 5.45], lookAt: [-0.1, 1.45, -1.35], fov: 50 } },
};
...
function certificateNeighbor(index: number, direction: FocusDirection): string | undefined {
  ...
  if (direction === "left" || direction === "right") {
    const target = CERTIFICATE_LAYOUT.find(
      (candidate) =>
        candidate.row === item.row &&
        candidate.column === item.column + (direction === "left" ? -1 : 1),
    );
    return target ? CERTIFICATES[target.index].slug : undefined;
  }
  const row = item.row + (direction === "up" ? -1 : 1);
  const candidates = CERTIFICATE_LAYOUT.filter((candidate) => candidate.row === row).sort(
...
const certificateItems = Object.fromEntries(
  CERTIFICATE_LAYOUT.map(({ index, x, y, row, column }) => {
    ...
    const item: FocusItemDefinition = {
      ...
      subject: `certificate:${certificate.slug}`,
      route: `/certificates/${certificate.slug}`,
      cameraTarget: "certificates",
      framing: SCENE_REGISTRY.certificates.framing,
      cameraFocus: SCENE_REGISTRY.certificates.cameraFocus,
      transition: SCENE_REGISTRY.certificates.transition,
      neighbors: {
        left: certificateNeighbor(index, "left"),
        right: certificateNeighbor(index, "right"),
        up: certificateNeighbor(index, "up"),
        down: certificateNeighbor(index, "down"),
      },
      metadata: {
        image: certificate.image,
        date: certificate.date,
        url: certificate.url,
        spatial: { x, y, row, column },
      },
    };
    return [item.id, item];
  }),
...
const dynamicCollection = (
  ...
  cameraTarget,
  exitBehavior: "start",
  defaultFraming: SCENE_REGISTRY[sceneId].framing,
  cameraFocus: SCENE_REGISTRY[sceneId].cameraFocus,
  transition: SCENE_REGISTRY[sceneId].transition,
  items: {},
  orderedItemIds: [],
...
export const FOCUS_COLLECTIONS: Record<string, FocusCollectionDefinition> = {
  certificates: {
    ...
    defaultFraming: SCENE_REGISTRY.certificates.framing,
    cameraFocus: SCENE_REGISTRY.certificates.cameraFocus,
    transition: SCENE_REGISTRY.certificates.transition,
    items: certificateItems,
    orderedItemIds: CERTIFICATES.map(({ slug }) => slug),
  },
...
export function getAdjacentScene(
  ...
  if (direction > 0) {
    ...
    while (nextIndex < GUIDED_SCENE_IDS.length) {
      ...
      if (!SCENE_REGISTRY[candidate].autoAdvance || !visitedAutoScenes.includes(candidate))
        return candidate;
      nextIndex += 1;
    }
    return null;
...
export function getFocusItem(collectionId: string, itemId: string): FocusItemDefinition | null {
  ...
  return {
    ...
    slug: itemId,
    label: itemId,
    subject: `${collection.sceneId}:${itemId}`,
    route: collection.routePattern.replace(":slug", itemId),
    cameraTarget: collection.cameraTarget,
    framing: collection.defaultFraming,
    cameraFocus: collection.cameraFocus,
    transition: collection.transition,
    neighbors: {},
  };
}

export function getFocusNeighbor(
  collectionId: string,
  itemId: string,
  direction: FocusDirection,
): FocusItemDefinition | null {
...
export function getAdjacentFocus(
  collectionId: string,
  itemId: string,
  direction: -1 | 1,
): FocusItemDefinition | null {
...
export function locationForScene(
  sceneId: SceneId,
  cameraTarget: ShotId = SCENE_REGISTRY[sceneId].cameraTarget,
): NavigationLocation {
...
export function locationForFocus(collectionId: string, itemId: string): NavigationLocation | null {
  ...
  return collection && item
    ? {
        ...
        focusItemId: item.id,
        cameraTarget:
          collection.reframeOnFocus === false
            ? SCENE_REGISTRY[collection.sceneId].cameraTarget
            : item.cameraTarget,
      }
    : null;
}
...
export function sceneForCameraTarget(target: ShotId): SceneId {
  ...
  const collection = Object.values(FOCUS_COLLECTIONS).find(
    ({ cameraTarget, items }) =>
      cameraTarget === target || Object.values(items).some((item) => item.cameraTarget === target),
  );
