...
function Arrow({direction}:{direction:"left"|"right"}){
  return <svg viewBox="0 0 24 24" aria-hidden="true"><path d={direction==="left"?"m14.5 6-6 6 6 6":"m9.5 6 6 6-6 6"}/></svg>;
}
...
export function SceneNavigation({selectedScene,selectedFocusCollection,selectedFocusItem,resumeScene,visitedAutoScenes,stateRef,onNavigate,onEnterFocus,onExitFocus}:Props){
  ...
  return <>
    <nav className="scene-navigation" aria-label="Scene navigation">
      <button type="button" className="scene-navigation-target scene-navigation-previous" aria-label={previous?`Previous scene: ${SCENE_REGISTRY[previous].label}`:"No previous scene"} disabled={!introComplete||!previous} onClick={()=>previous&&onNavigate(previous)}>
    ...
    {collection&&collection.orderedItemIds.length>0&&<nav className="collection-tab-navigation" aria-label={`${SCENE_REGISTRY[current].label} collection`}>
      {collection.orderedItemIds.map((itemId)=>{
        ...
        if(!item)return null;
        const active=selectedFocusCollection===collection.id&&selectedFocusItem===itemId;
        return <button key={itemId} type="button" aria-current={active?"true":undefined} onFocus={()=>{if(!active)onEnterFocus(collection.id,itemId);}} onClick={()=>focusItem(itemId)}>{item.label}</button>;
      })}
    </nav>}
  </>;
}
