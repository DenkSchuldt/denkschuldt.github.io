import {
  ...
  resolveShot,
  SHOT_REGISTRY,
  validateShotRegistry,
} from "./shotRegistry.ts";
...
const flatten = (id: CameraTargetId, aspect = 2): ResolvedCameraTarget => {
  const shot = resolveShot(id, aspect),
    frame = shot.framing;
  return {
    id: shot.id,
    label: shot.label,
    ...
    duration: shot.transition.duration,
    arrivalDelay: shot.transition.arrivalDelay,
    focusDistance: shot.focus.focusDistance,
    breathing: shot.transition.breathing,
  };
};
