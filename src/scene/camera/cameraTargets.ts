...
export const CAMERA_TARGETS: Record<CameraTargetId, CameraTarget> = {
  ...
  about: { id:"about", label:"About", position:[-1.45,3.08,.72], lookAt:[-1.28,1.25,-.88], fov:36, duration:4.3, focusDistance:.013, breathing:{...quietBreathing,positionAmplitude:.001}, waypoint:[-.7,2.85,2.15], responsive:{mobile:{position:[-1.2,3.35,1.25],fov:43}} },
...
export function resolveCameraTarget(id: CameraTargetId, aspect: number): ResolvedCameraTarget {
  ...
  return { ...target, position:override?.position ?? target.position, lookAt:override?.lookAt ?? target.lookAt, fov:override?.fov ?? target.fov };
}
