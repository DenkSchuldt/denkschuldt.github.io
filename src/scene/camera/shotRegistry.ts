import { FOCUS_COLLECTIONS, GUIDED_SCENE_IDS, SCENE_REGISTRY, WORKSPACE_PRESENTATION } from "./sceneRegistry.ts";
...
const sceneShot=(sceneId:keyof typeof SCENE_REGISTRY):Shot=>{
  ...
  return {id:scene.cameraTarget,label:scene.label,route:scene.route,subject:scene.subject,framing:scene.framing,focus:scene.cameraFocus,transition:scene.transition,responsive:scene.responsive,guided:true};
};

const focusShot=(id:ShotId,label:string,collectionId:keyof typeof FOCUS_COLLECTIONS,subject:string,route?:string):Shot=>{
  const collection=FOCUS_COLLECTIONS[collectionId];
  return {id,label,route:route??collection.routePattern,subject,framing:collection.defaultFraming,focus:collection.cameraFocus,transition:collection.transition,responsive:SCENE_REGISTRY[collection.sceneId].responsive};
};
...
export const SHOT_REGISTRY:Record<ShotId,Shot>={
  ...
  "certificate-detail":focusShot("certificate-detail","Certificate detail","certificates","certificate-detail"),
