...
export function CinematicFade({ replayKey, skipKey, hold, duration, reducedMotion, onComplete }: { replayKey:number; skipKey:number; hold:number; duration:number; reducedMotion:boolean; onComplete?:()=>void }) {
  const [visible, setVisible] = useState(true);
  useEffect(() => {
    setVisible(true);
    const delay = reducedMotion ? 80 : (hold * 1000);
    const timer = window.setTimeout(() => setVisible(false), delay);
    const completeTimer = window.setTimeout(() => onComplete?.(), delay + (reducedMotion ? 180 : duration * 1000));
    return () => { window.clearTimeout(timer); window.clearTimeout(completeTimer); };
