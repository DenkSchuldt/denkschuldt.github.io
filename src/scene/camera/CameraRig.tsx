...
export function CameraRig(props: Props) {
  ...
  useFrame(({clock}) => {
    ...
    if(!initialized.current){
      ...
      camera.position.copy(basePosition);camera.lookAt(baseLook);
    ...
    if(transitioning.current){
      ...
      camera.position.copy(targetPosition); camera.lookAt(baseLook);
    ...
    if(!props.breathingEnabled||props.reducedMotion||!breathing){camera.position.copy(basePosition);camera.lookAt(baseLook);return;}
    ...
    camera.lookAt(baseLook.x+Math.sin(now*speed*.37)*breathing.rotationAmplitude*props.breathingStrength,baseLook.y+Math.cos(now*speed*.29)*breathing.rotationAmplitude*props.breathingStrength,baseLook.z);
  });
  return null;
}
