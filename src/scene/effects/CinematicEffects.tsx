...
export default function CinematicEffects({
  ...
  useEffect(
    () => renderDemand.acquireFor({ reason: "effects-settle", priority: 2 }, 500),
    [composerActive, profile.id, readingMode, renderDemand],
  );
  useFrame(() =>
    measurePerformanceTask("CinematicEffectsFocusUpdate", () => {
      if (!features.depthOfField) return;
      const effect = dof.current as {
    ...
    <ManagedEffectComposer>
      {isolation.ambientOcclusion && features.ambientOcclusion && !blueprint ? (
        <N8AO
          ...
          distanceFalloff={profile.postprocessing.ao.distanceFalloff}
          quality={profile.postprocessing.ao.quality}
          halfRes={profile.postprocessing.ao.halfResolution}
        />
      ) : null}
      {features.depthOfField ? (
        <DepthOfField
          ref={dof}
      ...
      {features.grading ? (
        <HueSaturation
          hue={profile.postprocessing.grading.hue}
          saturation={profile.postprocessing.grading.saturation}
        />
      ) : null}
      {isolation.vignette && features.vignette ? (
        <Vignette
          eskil={false}
          offset={profile.postprocessing.vignette.offset}
          darkness={profile.postprocessing.vignette.darkness}
        />
      ) : null}
    </ManagedEffectComposer>
  );
