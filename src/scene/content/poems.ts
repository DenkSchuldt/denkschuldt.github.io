...
export interface PoemRecord extends PoemManifestEntry {
  body: string;
}
...
export function parsePoemFrontmatter(markdown: string, fallbackSlug: string): PoemFrontmatter {
  ...
  return {
    slug: rawSlug ? normalizePoemSlug(rawSlug, fallbackSlug) : null,
    title: scalar("title"),
    date: scalar("date"),
    language: scalar("lang"),
  };
}
...
export async function loadPoemManifest(
  fetcher: typeof fetch = fetch,
  endpoint = "/poems-manifest.json",
): Promise<PoemManifestEntry[]> {
...
export async function loadPoemContent(
