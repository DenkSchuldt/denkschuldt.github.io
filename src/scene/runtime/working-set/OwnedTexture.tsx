...
export function useOwnedTexture(url:string,resourceId:string,enabled=true){
  ...
  useEffect(()=>{
    ...
    new THREE.TextureLoader().load(url,(loaded)=>{
      ...
      owned=loaded;loaded.colorSpace=THREE.SRGBColorSpace;loaded.needsUpdate=true;
      try{gl.initTexture(loaded);}catch{}
      setTexture(loaded);
      store.resourceEvent("prepare-end",resourceId,{status:"resident",cache:"owned",detail:`${url}; decode complete, texture constructed, initTexture requested; first visible sample unverified`});
...
export function useOwnedTextures(urls:readonly string[],resourceId:string,enabled=true){
  ...
  useEffect(()=>{
    ...
    Promise.all(urls.map((url)=>new Promise<THREE.Texture>((resolve,reject)=>{
      ...
      loaded.forEach((texture)=>{try{gl.initTexture(texture);}catch{}});
      owned.push(...loaded);setTextures(loaded);
      store.resourceEvent("prepare-end",resourceId,{status:"resident",cache:"owned",detail:`${loaded.length} texture(s); decode complete, texture constructed, initTexture requested; first visible samples unverified`});
      renderDemand.invalidate("asset-ready");
    }).catch((error)=>{
