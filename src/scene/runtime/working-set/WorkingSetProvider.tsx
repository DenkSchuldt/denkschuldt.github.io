...
import { createResolverState, isResourceResidentState, resolveWorkingSet } from "./resolver";
...
class WorkingSetStore {
  ...
  private diagnostics = {
    ...
    logEvents: false,
    simulatedFailure: null as string | null,
  };
  ...
  private buildSnapshot(): WorkingSetSnapshot {
    ...
    return {
      ...
      approachingDestination: requested !== current ? requested : null,
      destinations: this.resolver.destinations,
      resources: this.resources,
      events: this.events,
      estimatedDecodedTextureBytes: Object.values(this.resources)
        .filter((resource) => resource.status !== "released")
        .reduce((sum, resource) => sum + (resource.estimatedDecodedBytes ?? 0), 0),
      pendingReleases: Object.values(this.resolver.destinations).filter(
        ({ releaseAt }) => releaseAt !== null,
      ).length,
      activeRuntimeTasks: this.taskCount,
      raycastCandidates: this.raycastCount,
  ...
  private log(event: WorkingSetEvent) {
    this.events = [...this.events, event].slice(-MAX_EVENTS);
    if (this.diagnostics.logEvents) console.info("[working-set]", event);
  }
  configureDiagnostics(search: string) {
    ...
    this.diagnostics = {
      force: params.get("wsForce"),
      retentionDisabled: params.get("wsRetention") === "0",
      showResourceIds: params.get("wsShowIds") === "1",
      logEvents: params.get("wsLog") === "1",
  ...
  update(intent: WorkingSetNavigationIntent, profileId: QualityProfileId, now = Date.now()) {
    ...
    if (this.diagnostics.retentionDisabled) {
      for (const destination of WORKING_SET_DESTINATIONS) {
        ...
        if (candidate.state === "sleeping" && candidate.reason !== "shared-session-retention")
          this.resolver.destinations[destination] = {
            ...candidate,
            state: "releasing",
            releaseAt: now,
            reason: "diagnostic-retention-disabled",
          };
      }
    }
    if (this.diagnostics.force) {
      ...
      if (
        WORKING_SET_DESTINATIONS.includes(destination as SceneId) &&
        ["absent", "ambient", "preparing", "active", "sleeping", "releasing", "error"].includes(
          state,
        )
      ) {
        const current = this.resolver.destinations[destination as SceneId];
        this.resolver.destinations[destination as SceneId] = {
          ...current,
          state: state as typeof current.state,
          reason: "diagnostic-force",
        };
      }
    }
    for (const destination of WORKING_SET_DESTINATIONS) {
      const before = previous[destination],
        after = this.resolver.destinations[destination];
      if (before.state !== after.state) {
        this.log({
          at: now,
          type: "state",
          resourceId: `destination:${destination}`,
          detail: `${before.state} -> ${after.state} (${after.reason})`,
        });
        if (after.state === "sleeping") {
          for (const resource of Object.values(this.resources))
            if (resource.destination === destination && resource.status === "resident")
              this.resources = {
                ...this.resources,
                [resource.id]: { ...resource, status: "sleeping" },
              };
        } else if (after.state === "active") {
          for (const resource of Object.values(this.resources))
            if (resource.destination === destination && resource.status === "sleeping")
              this.resources = {
                ...this.resources,
                [resource.id]: { ...resource, status: "resident" },
              };
        }
      }
    }
  ...
  private schedule() {
    ...
    const releases = Object.values(this.resolver.destinations)
      .map(({ releaseAt }) => releaseAt)
  ...
  resourceEvent(
    type: WorkingSetEvent["type"],
    resourceId: string,
    options: {
      status?: ResidentResourceState["status"];
      cache?: ResidentResourceState["cache"];
      estimatedDecodedBytes?: number | null;
      detail?: string;
      evidence?: readonly ReleaseEvidence[];
    } = {},
  ) {
    const definition = WORKING_SET_DESTINATIONS.flatMap(
      (destination) => WORKING_SET_DEFINITIONS[destination].resources,
    ).find(({ id }) => id === resourceId);
    if (definition && options.status)
      this.resources = {
        ...this.resources,
        [resourceId]: {
          id: resourceId,
          ...
          cache: options.cache ?? "owned",
          estimatedDecodedBytes:
            options.estimatedDecodedBytes ?? definition.estimatedDecodedBytes ?? null,
  ...
  setRuntimeMetrics(taskCount: number | null, raycastCount: number | null) {
    if (this.taskCount === taskCount && this.raycastCount === raycastCount) return;
  ...
  clearOwned() {
    for (const destination of WORKING_SET_DESTINATIONS) {
      ...
      this.resolver.destinations[destination] = {
        ...
        state: "releasing",
        releaseAt: Date.now(),
        reason: "diagnostic-owned-clear",
      };
    }
    this.log({
      at: Date.now(),
      type: "release",
      resourceId: "owned-boundaries",
      detail:
        "eligible sleeping boundaries moved to releasing; each owner must report its own dispose evidence",
    });
    this.commit();
  }
  dispose() {
...
export function useWorkingSetStore() {
  const store = useContext(WorkingSetContext);
  if (!store) throw new Error("useWorkingSetStore must be used inside WorkingSetProvider.");
  return store;
}
export function useWorkingSet<T>(selector: (snapshot: WorkingSetSnapshot) => T) {
...
export const useDestinationWorkingSet = (destination: SceneId) =>
  useWorkingSet((snapshot) => snapshot.destinations[destination]);
export const useDestinationResources = (destination: SceneId) =>
  useWorkingSet((snapshot) => isResourceResidentState(snapshot.destinations[destination].state));

export function WorkingSetNavigationAdapter({
  engine,
  profileId,
  overlayResourceIds = [],
}: {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const intent: WorkingSetNavigationIntent = {
        ...
        overlayResourceIds,
        focusedResourceIds: state.requestedFocusItemId ? [state.requestedFocusItemId] : [],
        visible: document.visibilityState !== "hidden",
      };
      store.update(intent, profileId);
      const nextState = Object.entries(store.getSnapshot().destinations)
        .map(([id, value]) => `${id}:${value.state}`)
        .join("|");
      if (nextState !== lastState) {
        lastState = nextState;
        renderDemand.invalidate("working-set-change");
      }
    };
    update();
    const failure = store.getSnapshot().diagnostics.simulatedFailure;
    if (failure)
      store.resourceEvent("error", failure, {
        status: "error",
        cache: "owned",
        detail: "simulated diagnostic failure",
      });
