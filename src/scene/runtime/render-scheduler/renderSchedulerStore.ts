...
const cadenceMs = (cadence: RenderLease["cadence"]) =>
  cadence === "15fps" ? 1000 / 15 : cadence === "30fps" ? 1000 / 30 : 0;
...
export class RenderSchedulerStore {
  ...
  setInvalidator(invalidator: (() => void) | null) {
  ...
  configure(search: string) {
    ...
    this.snapshot = {
      ...this.snapshot,
      forcedMode: mode === "continuous" ? "continuous" : mode === "demand" ? "demand" : null,
    };
    if (mode === "continuous")
      this.acquireContinuous({
        ownerId: "diagnostics",
        reason: "diagnostics-capture",
        priority: 3,
      });
  }
  invalidate(ownerId: string, reason: RenderReason) {
    ...
    const counts = {
      ...this.snapshot.invalidationsByOwner,
      [ownerId]: (this.snapshot.invalidationsByOwner[ownerId] ?? 0) + 1,
    };
    this.snapshot = {
      ...this.snapshot,
  ...
  acquireContinuous(request: RenderLeaseRequest, now = performance.now()) {
    ...
    if (existing) {
      ...
      this.commit();
      return () => this.release(key, renewed.generation);
    }
    const lease: RenderLease = {
      ...
      cadence: request.cadence ?? "display",
      startedAt: now,
      expiresAt: request.expiresAt ?? null,
      lifecycleId: request.lifecycleId ?? null,
      metadata: request.metadata ?? {},
      generation: ++this.generation,
    };
    this.leases.set(key, lease);
    this.commit();
    this.invalidate(request.ownerId, request.reason);
  ...
  acquirePeriodic(request: RenderLeaseRequest, now = performance.now()) {
    const release = this.acquireContinuous(
      { ...request, cadence: request.cadence ?? "15fps" },
      now,
    );
  ...
  releaseOwner(ownerId: string) {
    for (const [key, lease] of this.leases) if (lease.ownerId === ownerId) this.leases.delete(key);
    this.commit();
  }
  releaseLifecycle(lifecycleId: string) {
    for (const [key, lease] of this.leases)
      if (lease.lifecycleId === lifecycleId) this.leases.delete(key);
    this.commit();
  }
  ...
  setVisible(visible: boolean) {
    ...
    if (!visible) {
      ...
      this.invalidate("document", "visibility-restored");
      this.schedulePeriodic();
    }
    this.emit();
  }
  frame(now = performance.now()) {
    ...
    this.snapshot = {
      ...this.snapshot,
      renderedFrames: this.snapshot.renderedFrames + 1,
      framesWhileIdle: this.snapshot.framesWhileIdle + (logicallyIdle ? 1 : 0),
      pendingInvalidations: 0,
      lastRenderedAt: now,
  ...
  recordProjection() {
    this.snapshot = { ...this.snapshot, projectionUpdates: this.snapshot.projectionUpdates + 1 };
  }
  recordDof() {
    this.snapshot = { ...this.snapshot, dofUpdates: this.snapshot.dofUpdates + 1 };
  }
  warn(message: string) {
    if (process.env.NODE_ENV === "production") return;
    this.snapshot = { ...this.snapshot, warnings: [...this.snapshot.warnings, message].slice(-30) };
    this.emit();
  }
  clearExpired(now = performance.now()) {
    this.expire(now);
  }
  dispose() {
    if (this.periodicTimer) clearTimeout(this.periodicTimer);
  ...
  private release(key: string, generation: number) {
    const lease = this.leases.get(key);
    if (!lease || lease.generation !== generation) return;
    this.leases.delete(key);
    this.commit();
  }
  private periodicLeases() {
    return [...this.leases.values()].filter((lease) => lease.cadence !== "display");
  }
  private commit() {
    const continuous = [...this.leases.values()].filter((lease) => lease.cadence === "display"),
      periodic = this.periodicLeases();
    this.snapshot = {
      ...this.snapshot,
      ...
      idleSince:
        continuous.length || this.snapshot.pendingInvalidations
          ? null
          : (this.snapshot.idleSince ?? performance.now()),
    };
    this.schedulePeriodic();
    this.emit();
  }
  private schedulePeriodic() {
    ...
    this.periodicTimer = setTimeout(() => {
      this.periodicTimer = null;
      this.invalidate("periodic", leases[0].reason);
      this.schedulePeriodic();
    }, delay);
  }
  private emit() {
