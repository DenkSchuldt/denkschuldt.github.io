...
export function MeasuredRuntimeFrameBridge({runtime,paused=false}:{runtime:CinematicRuntime;paused?:boolean}){
  ...
  useFrame(({clock},delta)=>{
    if(paused)return;
    performanceDiagnostics.measure("RuntimeFrameBridge",()=>runtime.update(delta,clock.elapsedTime));
    if(clock.elapsedTime-lastMetrics.current>=1){lastMetrics.current=clock.elapsedTime;workingSet.setRuntimeMetrics(runtime.getScheduler().getActiveTaskCount(),workingSet.getSnapshot().raycastCandidates);}
  });
  return null;
}
...
export function PerformanceProbe(){
  ...
  useEffect(()=>{
    ...
    const debug=context.getExtension("WEBGL_debug_renderer_info") as {UNMASKED_VENDOR_WEBGL:number;UNMASKED_RENDERER_WEBGL:number}|null;
    const buffer=new THREE.Vector2();gl.getDrawingBufferSize(buffer);
    let shadowCasters=0,receivers=0,interactive=0;
    scene.traverse((object)=>{
      if((object as THREE.Mesh).castShadow)shadowCasters++;
      if((object as THREE.Mesh).receiveShadow)receivers++;
      const handlers=(object as THREE.Object3D&{__r3f?:{handlers?:Record<string,unknown>}}).__r3f?.handlers;
      if(handlers&&Object.keys(handlers).length)interactive++;
    });
    performanceDiagnostics.setEnvironment({
      userAgent:navigator.userAgent,platform:navigator.platform,viewport:{width:innerWidth,height:innerHeight},
      devicePixelRatio:window.devicePixelRatio,resolvedDpr:gl.getPixelRatio(),drawingBuffer:{width:buffer.x,height:buffer.y},
      ...
      maxTextureSize:context.getParameter(context.MAX_TEXTURE_SIZE),maxRenderbufferSize:context.getParameter(context.MAX_RENDERBUFFER_SIZE),
      maxTextureUnits:context.getParameter(context.MAX_TEXTURE_IMAGE_UNITS),precision:gl.capabilities.precision,
      maxAnisotropy:gl.capabilities.getMaxAnisotropy(),shadowMapType:gl.shadowMap.type,
      rendererInfoAutoResetBeforeDiagnostics:previousAutoReset,rendererInfoAutoResetDuringDiagnostics:gl.info.autoReset,shadowCasters,shadowReceivers:receivers,interactiveObjects:interactive,
    });
    const raycaster=THREE.Raycaster.prototype as THREE.Raycaster&{__portfolioOriginalIntersectObjects?:THREE.Raycaster["intersectObjects"]};
    if(!raycaster.__portfolioOriginalIntersectObjects){
      ...
      const measuredIntersect=function(this:THREE.Raycaster,objects:THREE.Object3D[],recursive=true,target:THREE.Intersection[]=[]){
        const start=performance.now(),before=target.length;
        const result=original.call(this,objects,recursive,target);
        performanceDiagnostics.pointer(performance.now()-start,objects.length,result.length-before);
        return result;
      };
  ...
  useFrame(({clock},delta)=>{
    ...
    performanceDiagnostics.measure("PerformanceProbe",()=>{
      ...
      performanceDiagnostics.addFrame({
        timestamp:performance.now(),deltaMs:delta*1000,calls:gl.info.render.calls,triangles:gl.info.render.triangles,
        points:gl.info.render.points,lines:gl.info.render.lines,geometries:gl.info.memory.geometries,textures:gl.info.memory.textures,
        programs:gl.info.programs?.length??null,drawingBufferWidth:buffer.x,drawingBufferHeight:buffer.y,
      });
      gl.info.reset();
      if(clock.elapsedTime-lastWorkingSetMetrics.current>=1){
        lastWorkingSetMetrics.current=clock.elapsedTime;
        let interactive=0;
        scene.traverse((object)=>{const handlers=(object as THREE.Object3D&{__r3f?:{handlers?:Record<string,unknown>}}).__r3f?.handlers;if(handlers&&Object.keys(handlers).length)interactive++;});
...
export function PerformanceOverlay(){
  ...
  useEffect(()=>{
    ...
    window.__PORTFOLIO_PERF__={
      reset:()=>performanceDiagnostics.reset(),label:(...args)=>performanceDiagnostics.label(...args),
      mark:(name)=>performanceDiagnostics.mark(name),invalidate:(reason)=>performanceDiagnostics.invalidate(reason),
      summary:()=>performanceDiagnostics.summary(),export:()=>performanceDiagnostics.export(),
    };
    const timer=window.setInterval(()=>performanceDiagnostics.emit(),500);
    return()=>{window.clearInterval(timer);delete window.__PORTFOLIO_PERF__;};
  },[]);
  ...
  return <aside aria-label="Performance diagnostics" data-performance-summary={JSON.stringify(summary)} data-quality-summary={JSON.stringify(qualitySummary)} data-working-set-summary={JSON.stringify(workingSet)} style={{position:"fixed",zIndex:100,right:12,top:12,width:340,maxHeight:"calc(100vh - 24px)",overflow:"auto",padding:12,border:"1px solid #ffffff2a",borderRadius:10,background:"#080808e8",color:"#eee",font:"11px/1.45 ui-monospace,monospace"}}>
    ...
    <div>DPR current / target: {quality.adaptive.currentDpr} / {quality.adaptive.targetDpr}</div>
    <div>Buffer: {quality.capabilities?.drawingBufferWidth??"n/a"} × {quality.capabilities?.drawingBufferHeight??"n/a"} ({quality.capabilities?Math.round(quality.capabilities.drawingBufferWidth*quality.capabilities.drawingBufferHeight/10000)/100:"n/a"} MP)</div>
    <div>Health median / p95: {quality.adaptive.health?.medianFrameMs.toFixed(2)??"n/a"} / {quality.adaptive.health?.p95FrameMs.toFixed(2)??"n/a"} ms</div>
    <div>Recent changes: {quality.adaptive.history.length}</div>
    <div>Working set: {workingSet.activeDestination}{workingSet.approachingDestination?` → ${workingSet.approachingDestination}`:""}</div>
    <div>Resident / preparing / sleeping: {Object.values(workingSet.resources).filter(({status})=>status==="resident").length} / {Object.values(workingSet.resources).filter(({status})=>status==="preparing").length} / {Object.values(workingSet.resources).filter(({status})=>status==="sleeping").length}</div>
    ...
    <div>Frame avg / p95: {summary.averageFrameMs.toFixed(2)} / {summary.p95FrameMs.toFixed(2)} ms</div>
    <div>Calls / triangles: {summary.latestRenderer?.calls??"n/a"} / {summary.latestRenderer?.triangles??"n/a"}</div>
    <div>Textures / geometries / programs: {summary.latestRenderer?.textures??"n/a"} / {summary.latestRenderer?.geometries??"n/a"} / {summary.latestRenderer?.programs??"n/a"}</div>
