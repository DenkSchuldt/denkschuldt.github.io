...
export function QualityRuntimeBridge({transitioning,overlayChanging}:{transitioning:boolean;overlayChanging:boolean}){
  ...
  useEffect(()=>{samples.current=[];store.resetViewport(detectPreliminaryCapabilities());setDpr(store.getSnapshot().adaptive.currentDpr);},[size.width,size.height,setDpr,store]);
  useFrame((_,delta)=>{
    ...
    if(result.nextDpr!==null)setDpr(result.nextDpr);
