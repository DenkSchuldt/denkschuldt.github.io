...
const CERTIFICATE_SOURCES: readonly CertificateSource[] = [
  {
...
export const CERTIFICATES: readonly CertificateRecord[] = CERTIFICATE_SOURCES.map((record) => ({
  ...record,
  slug: slugifyCertificate(record.title),
}));

export const certificateImagePath = (certificate: CertificateRecord) =>
  `/certificates/${certificate.image}`;

export function certificateIssuer(certificate: CertificateRecord): string | null {
  if (certificate.url.includes("interaction-design.org")) return "Interaction Design Foundation";
  if (certificate.url.includes("online.hbs.edu")) return "Harvard Business School Online";
  return null;
}
