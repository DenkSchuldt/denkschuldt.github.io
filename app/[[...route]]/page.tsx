...
const absoluteUrl = (pathname: string) =>
  `${SITE_ORIGIN}${BASE_PATH}${pathname.startsWith("/") ? pathname : `/${pathname}`}`;
const excerpt = (body: string) =>
  body
    ...
    .slice(0, 180)
    .replace(/\s+\S*$/, "")
    .concat(body.replace(/\s+/g, " ").trim().length > 180 ? "…" : "");
...
export async function generateMetadata({
  ...
  if (!route[1]) {
    const title = "Poems — Denny K. Schuldt",
      description =
        "Original poetry by Denny K. Schuldt, presented as a cinematic writing portfolio.",
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      url,
      images: [{ url: image, alt: `Preview of ${poem.title}` }],
...
export default async function WorldPage({ params }: { params: Promise<{ route?: string[] }> }) {
  ...
  const structuredData = poem
    ? JSON.stringify({
        ...
        image: absoluteUrl(poem.imagePath),
        author: {
          "@type": "Person",
          name: "Denny K. Schuldt",
          url: "https://denkschuldt.github.io/",
        },
        copyrightHolder: { "@type": "Person", name: "Denny K. Schuldt" },
  ...
  return (
    <>
      ...
      {route[0] === "poems" && !poem && (
        <section className="poem-share-preview" aria-label="Poems by Denny K. Schuldt">
          <h1>Poems by Denny K. Schuldt</h1>
          <p>Original poetry, newest first.</p>
          <ol>
            {poems.map((entry) => (
              <li key={entry.slug}>
                <a href={`${BASE_PATH}/poems/${entry.slug}`}>{entry.title}</a>{" "}
                <time dateTime={entry.date}>{entry.date}</time>
              </li>
            ))}
          </ol>
          <a href={`${BASE_PATH}/poems/feed.xml`} type="application/atom+xml">
      ...
      {poem && (
        <article
          className="poem-share-preview"
          aria-label={`Poem: ${poem.title}`}
          lang={poem.language}
        >
          <h1>{poem.title}</h1>
          <p>{poem.body}</p>
          <img src={`${BASE_PATH}${poem.imagePath}`} alt={`Artwork for ${poem.title}`} />
          <nav aria-label="Poem navigation">
            {poems[poemIndex - 1] && (
              <a rel="prev" href={`${BASE_PATH}/poems/${poems[poemIndex - 1].slug}`}>
                {poems[poemIndex - 1].title}
              </a>
            )}
            <a href={`${BASE_PATH}/poems`}>All poems</a>
            {poems[poemIndex + 1] && (
              <a rel="next" href={`${BASE_PATH}/poems/${poems[poemIndex + 1].slug}`}>
                {poems[poemIndex + 1].title}
              </a>
            )}
          </nav>
          <a rel="alternate" type="text/markdown" href={`${BASE_PATH}${poem.contentUrl}`}>
