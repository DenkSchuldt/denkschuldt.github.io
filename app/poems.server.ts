...
import { normalizePoemSlug,parsePoemFrontmatter,parsePoemMarkdown } from "../src/scene/content/poems.ts";
...
export async function getStaticPoemPreviews():Promise<StaticPoemPreview[]> {
  ...
  const poems=(await Promise.all(folders.filter((entry)=>entry.isDirectory()).map(async(entry)=>{
...
const siteOrigin=()=>`${(process.env.NEXT_PUBLIC_SITE_URL??"https://denkschuldt.github.io").replace(/\/$/,"")}${(process.env.NEXT_PUBLIC_BASE_PATH??"").replace(/\/$/,"")}`;
const absoluteUrl=(pathname:string)=>`${siteOrigin()}${pathname.startsWith("/")?pathname:`/${pathname}`}`;
const escapeXml=(value:string)=>value.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&apos;");
const summary=(body:string,limit=240)=>{const text=body.replace(/\s+/g," ").trim();return text.length<=limit?text:`${text.slice(0,limit).replace(/\s+\S*$/,"")}…`;};
...
export async function getPoemsSitemapXml(){
  ...
  const urls=[...scenes.map((pathname)=>({url:absoluteUrl(pathname),lastmod:pathname==="/poems"?poems[0]?.date:undefined})),...poems.map((poem)=>({url:absoluteUrl(`/poems/${poem.slug}`),lastmod:poem.date}))];
  return `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls.map(({url,lastmod})=>`  <url>\n    <loc>${escapeXml(url)}</loc>${lastmod?`\n    <lastmod>${escapeXml(lastmod)}</lastmod>`:""}\n  </url>`).join("\n")}\n</urlset>\n`;
}

export async function getPoemsAtomFeed(){
  const poems=await getStaticPoemPreviews();
  const collectionUrl=absoluteUrl("/poems"),feedUrl=absoluteUrl("/poems/feed.xml");
  const updated=`${poems[0]?.date??"2023-01-01"}T00:00:00Z`;
  const entries=poems.map((poem)=>{
    const url=absoluteUrl(`/poems/${poem.slug}`),published=`${poem.date}T00:00:00Z`;
