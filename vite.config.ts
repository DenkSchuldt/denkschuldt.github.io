...
import { getLlmsText,getPoemsAtomFeed,getPoemsSitemapXml,getRobotsText,getStaticPoemManifest } from "./app/poems.server";
  ...
  configureServer(server){
    server.middlewares.use("/poems-manifest.json",async(_request,response)=>{
      try{
        ...
        response.setHeader("Content-Type","application/json; charset=utf-8");
        response.end(JSON.stringify({poems:[],error:error instanceof Error?error.message:String(error)}));
      }
    });
  },
  async generateBundle(){
    const [poems,sitemap,feed,llms]=await Promise.all([getStaticPoemManifest(),getPoemsSitemapXml(),getPoemsAtomFeed(),getLlmsText()]);
    this.emitFile({type:"asset",fileName:"poems-manifest.json",source:JSON.stringify({poems})});
