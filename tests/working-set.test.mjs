...
import {
  WORKING_SET_DEFINITIONS,
  WORKING_SET_DESTINATIONS,
} from "../src/scene/runtime/working-set/definitions.ts";
import {
  createResolverState,
  isPreparationGenerationCurrent,
  resolvePreparationPriority,
  resolveWorkingSet,
} from "../src/scene/runtime/working-set/resolver.ts";
...
test("every cinematic destination has an explicit definition", () => {
  assert.deepEqual(
    new Set(WORKING_SET_DESTINATIONS),
    new Set(["opening", "about", "certificates", "projects", "wall", "phone", "poems", "drawer"]),
  );
...
test("hidden tabs do not advance timed release", () => {
  ...
  state = resolveWorkingSet(
    state,
    intent("about", "about", false, { visible: false }),
    "mobile",
    99999,
  );
  assert.equal(state.destinations.phone.state, "sleeping");
...
test("overlay and focus resources outrank destinations", () => {
  const priorities = resolvePreparationPriority(
    intent("poems", "phone", true, {
      overlayResourceIds: ["poem-markdown"],
      focusedResourceIds: ["poem:selected"],
    }),
    "mobile",
  );
  assert.deepEqual(
    priorities.map(({ id }) => id),
    ["poem-markdown", "poem:selected", "destination:poems", "destination:phone"],
  );
});
...
test("mobile retains fewer resources than ultra and fallback disables retention", () => {
  assert.ok(
    WORKING_SET_DEFINITIONS.certificates.retentionMs.mobile <
      WORKING_SET_DEFINITIONS.certificates.retentionMs.ultra,
  );
...
test("shared resources declare session ownership and are never locally evicted", () => {
  const shared = Object.values(WORKING_SET_DEFINITIONS)
  ...
  assert.ok(
    shared.every(
      (resource) =>
        resource.class === "ambient" ||
        resource.class === "persistent-essential" ||
        resource.class === "shared-cache",
    ),
  );
});
