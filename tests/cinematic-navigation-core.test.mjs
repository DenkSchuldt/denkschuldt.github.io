...
const scene = (id) => ({
  ...
  cameraTargetId: `camera:${id}`,
  framing: { id },
  transition: { duration: 1 },
  ...(id === "two"
    ? { revisitTransition: { duration: 0.4 }, returnTransition: { duration: 0.6 } }
    : {}),
});
const item = (id, x, y, neighbors = {}) => ({
  id,
  subjectId: `subject:${id}`,
  cameraTargetId: "camera:detail",
  framing: { id },
  transition: { duration: 1 },
  spatial: { x, y },
  neighbors,
    ...
    focusCollections: [
      {
        ...
        items: [
          item("a", 0, 1, { right: "b" }),
          item("b", 1, 1, { left: "a", down: "c" }),
          item("c", 1, 0, { up: "b" }),
        ],
      },
    ],
    guidedSequence: ["one", "two", "three"],
    guidedWrap: "forward",
    initialLocation: {
      sceneId: "one",
      focusCollectionId: null,
      focusItemId: null,
      cameraTargetId: "camera:one",
    },
    ...overrides,
  });
...
test("registers scenes, rejects duplicates, and cleans registrations", () => {
  ...
  const unregister = engine.registerScene(scene("temporary"));
  unregister();
...
test("enters, moves, and exits Focus without leaving its Scene", () => {
  ...
  assert.deepEqual(engine.exitFocus(), {
    sceneId: "two",
    focusCollectionId: null,
    focusItemId: null,
    cameraTargetId: "camera:two",
  });
});
...
test("static Focus collections keep the parent camera while changing items", () => {
  ...
  const engine = createEngine({
    cameraDriver: {
      start: (request) => calls.push(["start", request.cameraTargetId]),
      redirect: (request) => calls.push(["redirect", request.cameraTargetId]),
      cancel: () => {},
    },
    focusCollections: [
      {
  ...
  assert.deepEqual(engine.enterFocus("cards", "a"), {
    sceneId: "two",
    focusCollectionId: "cards",
    focusItemId: "a",
    cameraTargetId: "camera:two",
  });
  assert.deepEqual(engine.moveFocus(1), {
    sceneId: "two",
    focusCollectionId: "cards",
    focusItemId: "b",
    cameraTargetId: "camera:two",
...
test("clearing a static focus keeps the parent camera stable", () => {
  ...
  assert.deepEqual(
    ...
    { sceneId: "two", focusCollectionId: null, focusItemId: null, cameraTargetId: "camera:two" },
  );
  assert.equal(calls.length, 0);
  assert.equal(engine.getState().transitionStatus, "idle");
});
...
test("explicit neighbors win and spatial fallback selects the closest candidate", () => {
  const engine = createCinematicEngine({
    ...
    focusCollections: [
      {
        id: "art",
        sceneId: "gallery",
        cameraTargetId: "detail",
        framing: {},
        transition: {},
        items: [
          item("origin", 0, 0, { right: "far" }),
          item("near", 1, 0),
          item("far", 3, 0),
          item("upper", 0.1, 2),
        ],
    ...
    initialLocation: {
      ...
      focusCollectionId: null,
      focusItemId: null,
      cameraTargetId: "camera:gallery",
    },
  });
  engine.enterFocus("art", "origin");
  assert.equal(engine.moveFocus("right").focusItemId, "far");
  engine.enterFocus("art", "origin");
  assert.equal(engine.moveFocus("up").focusItemId, "upper");
});
...
test("redirects transitions and keeps logical/camera completion explicit", () => {
  ...
  const engine = createEngine({
    cameraDriver: {
      start: (request) => calls.push(["start", request.sceneId]),
      redirect: (request) => calls.push(["redirect", request.sceneId]),
  ...
  assert.equal(engine.getState().sceneId, "one");
  engine.updateTransition(0.5);
  engine.completeTransition();
  assert.equal(engine.getState().sceneId, "three");
  assert.equal(engine.getState().transitionStatus, "idle");
...
test("replays the last settled camera location to late scene-focus subscribers", () => {
  ...
  engine.completeTransition();
  const focused = [];
  const unsubscribe = engine.onSceneFocused((state) =>
    focused.push({ sceneId: state.sceneId, cameraTargetId: state.cameraTargetId }),
  );
  assert.deepEqual(focused, [{ sceneId: "two", cameraTargetId: "camera:two" }]);
  engine.goToScene("three");
  engine.completeTransition();
  assert.deepEqual(focused, [
    { sceneId: "two", cameraTargetId: "camera:two" },
    { sceneId: "three", cameraTargetId: "camera:three" },
  ]);
  unsubscribe();
});
...
test("resolves revisit transitions only after a Scene has been reached", () => {
  ...
  assert.deepEqual(engine.resolveSceneTransition("two"), {
    sceneId: "two",
    transition: { duration: 1 },
    revisit: false,
    variant: "base",
  ...
  assert.deepEqual(engine.resolveSceneTransition("one"), {
    sceneId: "one",
...
test("persistence is injected and explicit route state remains authoritative", () => {
  ...
  const engine = createEngine({ persistence, persistenceKey: "last" });
  engine.goToScene("three");
  engine.completeTransition();
  const restored = createEngine({ persistence, persistenceKey: "last" });
  restored.syncLocation({
    sceneId: "two",
    focusCollectionId: null,
    focusItemId: null,
    cameraTargetId: "camera:two",
  ...
  assert.throws(
    () =>
      createCinematicEngine({
        scenes: [scene("one")],
        focusCollections: [
          { id: "broken", sceneId: "missing", cameraTargetId: "x", framing: {}, transition: {} },
        ],
      }),
    /Missing parent Scene/,
  );
  const engine = createEngine();
  assert.throws(() => engine.goToScene("missing"), /unregistered Scene/);
  assert.throws(() => engine.enterFocus("missing", "x"), /unregistered collection/);
});
