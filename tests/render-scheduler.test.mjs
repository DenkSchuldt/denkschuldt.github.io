...
test("one-shot invalidation is attributed and consumed by one frame",()=>{
  ...
  let requested=0;store.setInvalidator(()=>requested++);
...
test("first continuous lease activates rendering and final release returns idle",()=>{
  const store=new RenderSchedulerStore();
  const release=store.acquireContinuous({ownerId:"camera",reason:"navigation-transition"},0);
...
test("stale expiry cannot release a renewed lease",()=>{
  const store=new RenderSchedulerStore();
  store.acquireContinuous({ownerId:"navigation",reason:"navigation-transition",expiresAt:5},0);
  store.acquireContinuous({ownerId:"navigation",reason:"navigation-transition",expiresAt:50},2);
  store.expire(6);
  assert.equal(store.getSnapshot().continuousLeases.length,1);
  store.expire(51);
  assert.equal(store.getSnapshot().continuousLeases.length,0);
...
test("owner and lifecycle cleanup release leases idempotently",()=>{
  const store=new RenderSchedulerStore();
  store.acquireContinuous({ownerId:"phone",reason:"phone-screen",lifecycleId:"scene:phone"},0);
...
test("hidden documents ignore invalidations and restore with one refresh",()=>{
  const store=new RenderSchedulerStore();
  let requested=0;store.setInvalidator(()=>requested++);
  store.setVisible(false);store.invalidate("resize","resize");
...
test("projection and DOF diagnostics count real executions",()=>{
  const store=new RenderSchedulerStore();
  store.recordProjection();store.recordProjection();store.recordDof();
...
test("bounded feature lease expires without releasing a renewal",()=>{
  ...
  store.expire(99);assert.equal(store.getSnapshot().continuousLeases.length,1);
  store.acquireFor({ownerId:"poems",reason:"poems-preview"},100,90);
  store.expire(101);assert.equal(store.getSnapshot().continuousLeases.length,1);
  store.expire(191);assert.equal(store.getSnapshot().continuousLeases.length,0);
  store.dispose();
});
