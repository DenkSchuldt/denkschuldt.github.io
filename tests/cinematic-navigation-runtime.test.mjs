...
} from "../packages/cinematic-navigation/dist/core/index.js";
...
test("shared node declarations are reference counted and lifecycle events are coarse", () => {
  ...
  const phases = [];
  const unsubscribe = runtime.subscribeNode(node.id, (next, previous) =>
    phases.push([previous?.phase ?? null, next?.phase ?? null]),
  );
