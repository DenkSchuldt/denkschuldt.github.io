...
test("R3F subjects resolve live handles and clean up safely",()=>{
  const registry=createSubjectRegistry(),subject=new Object3D();
...
test("normalized input dispatches without owning event listeners",()=>{
  const calls=[];
  const engine={nextScene:()=>calls.push("next"),previousScene:()=>calls.push("previous"),enterFocus:(collection,item)=>calls.push(`${collection}:${item}`),goToFocus:(item)=>calls.push(item),exitFocus:()=>calls.push("exit"),moveFocus:(direction)=>calls.push(direction),interruptTransition:()=>calls.push("interrupt")};
