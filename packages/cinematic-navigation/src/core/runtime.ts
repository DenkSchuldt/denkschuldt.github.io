...
export function createCinematicRuntime(engine:CinematicEngine):CinematicRuntime {
  const nodes=new Map<string,RuntimeNodeRegistration>(),listeners=new Set<()=>void>();
  ...
  const runtime:CinematicRuntime={
    registerNode(node:RuntimeNodeRegistration){
      ...
      if(existing&&existing!==node)throw new Error(`[cinematic-navigation/runtime] Duplicate node ID: ${node.id}`);
      nodes.set(node.id,node);refresh();
      return()=>{if(nodes.get(node.id)===node){nodes.delete(node.id);refresh();}};
    },
    unregisterNode(nodeId:string){if(nodes.delete(nodeId))refresh();},
    ...
    dispose(){unsubscribeEngine();nodes.clear();scheduler.clear();listeners.clear();},
    subscribe(listener:()=>void){listeners.add(listener);return()=>listeners.delete(listener);},
  };
  const refresh=()=>{snapshot=createSnapshot(engine.getState(),nodes.values());listeners.forEach((listener)=>listener());};
