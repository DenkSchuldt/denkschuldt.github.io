...
export type RuntimeNodeListener = (
  next: RuntimeNodeState | undefined,
  previous: RuntimeNodeState | undefined,
) => void;
...
function deriveScenePhase(node: RuntimeNodeRegistration, state: EngineState): RuntimePhase {
  if (
    node.mountPolicy === "lazy" &&
    state.requestedSceneId !== node.sceneId &&
    state.sceneId !== node.sceneId
  )
    return "disposed";
  if (state.requestedSceneId === node.sceneId && state.sceneId !== node.sceneId)
    return state.transitionStatus === "transitioning" ? "transitioning-in" : "active";
  if (state.sceneId === node.sceneId && state.requestedSceneId !== node.sceneId)
    return state.transitionStatus === "transitioning" ? "exiting" : "sleeping";
...
function deriveCollectionPhase(node: RuntimeNodeRegistration, state: EngineState): RuntimePhase {
  ...
  if (
    state.focusCollectionId === node.collectionId &&
    state.requestedFocusCollectionId !== node.collectionId
  )
    return "exiting";
  return state.requestedSceneId === node.sceneId && state.transitionStatus === "transitioning"
    ? "transitioning-in"
    : "active";
}
...
function deriveFocusPhase(node: RuntimeNodeRegistration, state: EngineState): RuntimePhase {
  if (
    state.requestedFocusCollectionId === node.collectionId &&
    state.requestedFocusItemId === node.focusItemId
  ) {
    return state.transitionStatus === "transitioning" ? "transitioning-in" : "focused";
  }
  if (state.focusCollectionId === node.collectionId && state.focusItemId === node.focusItemId)
    return "transitioning-out";
  return node.mountPolicy === "lazy" ? "disposed" : "inactive";
}

export function deriveRuntimeNodeState(
  node: RuntimeNodeRegistration,
  state: EngineState,
): RuntimeNodeState {
  ...
  const mounted =
    phase !== "disposed" && (retained || (phase !== "sleeping" && phase !== "inactive"));
  ...
  return {
    ...
    active,
    interactive:
      (active && node.scope !== "scene") || (phase === "active" && node.scope === "scene"),
    updates: active || transitioning,
    contributesToRender: mounted && phase !== "disposed",
  };
}

function createSnapshot(
  state: EngineState,
  nodes: Iterable<RuntimeNodeRegistration>,
): RuntimeSnapshot {
...
class Scheduler implements RuntimeScheduler {
  ...
  constructor(getSnapshot: () => RuntimeSnapshot) {
    this.#getSnapshot = getSnapshot;
  }
  register(task: RuntimeTaskRegistration) {
    if (this.#tasks.has(task.id))
      throw new Error(`[cinematic-navigation/runtime] Duplicate task ID: ${task.id}`);
  ...
  update(delta: number, elapsed: number) {
    ...
    (this.#orderedTasks ??= Array.from(this.#tasks.values()).sort(
      (a, b) => (a.priority ?? 0) - (b.priority ?? 0),
    )).forEach((task) => {
      if (active.has(task.nodeId)) task.update({ delta, elapsed, runtime: snapshot });
    });
  }
  getActiveTaskCount() {
    const snapshot = this.#getSnapshot(),
      active = new Set(snapshot.nodes.filter((node) => node.updates).map((node) => node.id));
...
export function createCinematicRuntime(engine: CinematicEngine): CinematicRuntime {
  const nodes = new Map<string, RuntimeNodeRegistration>(),
    nodeReferences = new Map<string, number>(),
    listeners = new Set<() => void>(),
    nodeListeners = new Map<string, Set<RuntimeNodeListener>>();
  let snapshot = createSnapshot(engine.getState(), nodes.values());
  const scheduler = new Scheduler(() => snapshot);
  const equivalentNode = (left: RuntimeNodeRegistration, right: RuntimeNodeRegistration) =>
    left.id === right.id &&
    left.scope === right.scope &&
    left.sceneId === right.sceneId &&
    left.collectionId === right.collectionId &&
    left.focusItemId === right.focusItemId &&
    left.mountPolicy === right.mountPolicy &&
    left.retainOnSleep === right.retainOnSleep;
  const lifecycleChanged = (
    left: RuntimeNodeState | undefined,
    right: RuntimeNodeState | undefined,
  ) =>
    !left ||
    !right ||
    ...
    left.contributesToRender !== right.contributesToRender;
  const runtime: CinematicRuntime = {
    registerNode(node: RuntimeNodeRegistration) {
      const existing = nodes.get(node.id);
      if (existing && !equivalentNode(existing, node))
        throw new Error(`[cinematic-navigation/runtime] Conflicting node registration: ${node.id}`);
      if (existing) nodeReferences.set(node.id, (nodeReferences.get(node.id) ?? 1) + 1);
      else {
        nodes.set(node.id, node);
        nodeReferences.set(node.id, 1);
        refresh();
      }
      return () => {
        const references = nodeReferences.get(node.id) ?? 0;
        if (references <= 1) {
          nodeReferences.delete(node.id);
          if (nodes.get(node.id) === existing || nodes.get(node.id) === node) {
    ...
    update(delta: number, elapsed: number) {
      scheduler.update(delta, elapsed);
    },
    getNodeState(nodeId: string) {
      return snapshot.nodes.find((node) => node.id === nodeId);
    },
    subscribeNode(nodeId: string, listener: RuntimeNodeListener) {
      const subscriptions = nodeListeners.get(nodeId) ?? new Set<RuntimeNodeListener>();
      subscriptions.add(listener);
      nodeListeners.set(nodeId, subscriptions);
      return () => {
        subscriptions.delete(listener);
        if (!subscriptions.size) nodeListeners.delete(nodeId);
      };
    },
    getSnapshot() {
      return snapshot;
  ...
  const refresh = () => {
    const previous = snapshot,
      next = createSnapshot(engine.getState(), nodes.values());
    const changed =
      next.nodes.some((node) =>
        lifecycleChanged(
          node,
          previous.nodes.find((candidate) => candidate.id === node.id),
        ),
      ) || previous.nodes.some((node) => !next.nodes.some((candidate) => candidate.id === node.id));
    snapshot = next;
    if (!changed) return;
    ...
    previous.nodes.forEach((node) => {
      if (next.nodes.some((candidate) => candidate.id === node.id)) return;
      nodeListeners.get(node.id)?.forEach((listener) => listener(undefined, node));
    });
