...
import { createCinematicRuntime } from "../core/index.js";
...
const hotReloadGlobals = globalThis as typeof globalThis & {
  __DENK_CINEMATIC_NAVIGATION_CONTEXTS__?: SharedContexts;
};
...
export function CinematicEngineProvider({
  engine,
  children,
}: PropsWithChildren<{ engine: CinematicEngine }>) {
  return createElement(EngineContext.Provider, { value: engine }, children);
}

export function useCinematicEngine() {
  const engine = useContext(EngineContext);
  if (!engine) throw new Error("useCinematicEngine must be used inside CinematicEngineProvider.");
  return engine;
}

export function useCinematicState<T = EngineState>(
  selector: (state: Readonly<EngineState>) => T = (state) => state as T,
) {
...
export const useCurrentFocus = () =>
  useCinematicState((state) => ({
    collectionId: state.requestedFocusCollectionId,
    itemId: state.requestedFocusItemId,
  }));

export function useSceneRegistration(scene: SceneRegistration) {
  const engine = useCinematicEngine();
  useEffect(() => engine.registerScene(scene), [engine, scene]);
}

export function useFocusCollectionRegistration(collection: FocusCollectionRegistration) {
  const engine = useCinematicEngine();
  useEffect(() => engine.registerFocusCollection(collection), [engine, collection]);
}

export function useFocusItemRegistration(collectionId: string, item: FocusItemRegistration) {
  const engine = useCinematicEngine();
...
export function CinematicRuntimeProvider({
  runtime,
  children,
}: PropsWithChildren<{ runtime: CinematicRuntime }>) {
...
export function useCinematicRuntime() {
  ...
  if (!runtime)
    throw new Error("useCinematicRuntime must be used inside CinematicRuntimeProvider.");
  return runtime;
}

export function useRuntimeSnapshot<T = RuntimeSnapshot>(
  selector: (snapshot: RuntimeSnapshot) => T = (snapshot) => snapshot as T,
) {
...
export function useRuntimeNode(node: RuntimeNodeRegistration): RuntimeNodeState | undefined {
  ...
  return useRuntimeSnapshot((snapshot) =>
    snapshot.nodes.find((candidate) => candidate.id === node.id),
  );
}
...
export function useRuntimeNodeLifecycle(nodeId: string, listener: RuntimeNodeListener) {
  ...
  useEffect(
    () => runtime.subscribeNode(nodeId, (next, previous) => listenerRef.current(next, previous)),
    [runtime, nodeId],
  );
}
...
export function useRuntimeTask(task: RuntimeTaskRegistration) {
  ...
  const stableTask = useMemo<RuntimeTaskRegistration>(
    () => ({
      id: task.id,
      nodeId: task.nodeId,
      priority: task.priority,
      update: (context) => taskRef.current.update(context),
    }),
    [task.id, task.nodeId, task.priority],
  );
...
export function RuntimeInspector({
  ...
  visible?: boolean;
  metrics?: RuntimeInspectorMetrics;
}) {
...
function RuntimeInspectorView({ metrics }: { metrics?: RuntimeInspectorMetrics }) {
  ...
  const groups = (phase: string) =>
    snapshot.nodes.filter((node) => node.phase === phase).map((node) => node.id);
  ...
  return (
    <aside
      aria-label="Runtime lifecycle inspector"
      style={{
        position: "fixed",
        zIndex: 40,
        right: 16,
    ...
    >
      ...
      {line(
        ...
        snapshot.nodes
          .filter(
            (node) =>
              node.phase === "transitioning-in" ||
              node.phase === "transitioning-out" ||
              node.phase === "exiting",
          )
          .map((node) => node.id)
          .join(", ") || "—",
      ...
      {line(
        "Mounted scenes",
        snapshot.nodes
          .filter((node) => node.scope === "scene" && node.mounted)
          .map((node) => node.id)
          .join(", ") || "—",
      )}
      {line(
        "Mounted collections",
        snapshot.nodes
          .filter((node) => node.scope === "collection" && node.mounted)
          .map((node) => node.id)
          .join(", ") || "—",
      )}
      {line(
