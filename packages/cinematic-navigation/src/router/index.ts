...
export const normalizeBasename=(basename:string)=>basename==="/"?"":basename.replace(/\/$/,"");
export const addBasename=(path:string,basename="")=>`${normalizeBasename(basename)}${path==="/"?"/":path}`;
export function removeBasename(path:string,basename=""){
  const base=normalizeBasename(basename);if(!base)return path;
